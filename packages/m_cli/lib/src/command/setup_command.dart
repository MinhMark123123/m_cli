...
class SetupCommand extends BaseCommand {
  ...
  Future<void> onCommandExecuted() async {
    ...
    switch (sourceType) {
      ...
      case SourceType.path:
        return _handlePathSetup(name: name);
      case SourceType.unknow:
        updateEndingMessage(
          "You haven't input any source! please try m setup --source=<git/zip/path>",
          type: TextType.error,
        );
        return;
    }
  ...
  Future<void> _handlePathSetup({String? name}) async {
    final pathOptions = getOptionAt(2);
    final path = pathOptions.getArgOption<String?>(
      argResults: argResults,
    );
    final lastMessage = await _pathSettingManager.handleNewSetting(
      name: name,
      setting: path,
    );
    updateEndingMessage(lastMessage.$1, type: lastMessage.$2);
