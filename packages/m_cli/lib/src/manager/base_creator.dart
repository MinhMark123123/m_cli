...
abstract class BaseCreator {
  ...
  Future<(String, TextType?)> create({
    ...
    final settingSource = setting.getAvailableSource();
    if (settingSource != SourceType.path) {
    ...
    final createDirPath = await FileUtils.getCreateDir(
      name: key,
      type: settingSource,
      localPath: settingSource == SourceType.path ? setting.path : null,
    );
    ...
    final destinationDir = outputPath?.isNotEmpty == true
  ...
  Future<(String, TextType)> excutedCreate({
    ...
    //copy template
    FileUtils.copyDirectorySync(
      source: source,
      destination: destination,
      onConflictCondition: (file) {
        final result = ConsoleUtils.echoConfirmYnNnA(
          message: "${file.path} Conflict detected do you want to overwrite:",
        );
        return result;
      },
    );
    final metadataPath = FileUtils.getMetadataPath(srcPath: destination.path);
    final metadata = await readMetadata(metadataPath);
    Map<String, (String, bool)> variablesMap = {};
      ...
      for (var e in metadata.variables!) {
        if (e.prompt?.isNotEmpty == true && e.key?.isNotEmpty == true) {
          final input = ConsoleUtils.echoAndRead(
              ...
              e.defaultValue;
          variablesMap[e.key!] = (input!, e.reCase ?? true);
        }
      }
    }
  ...
  void _composeContent(
    ...
    for (var key in variablesMap.keys) {
      final listKeys = CaseType.converts(key);
      final values = variablesMap[key]!.$2
          ? CaseType.converts(variablesMap[key]!.$1) //reCase is true
          : List.generate(
              listKeys.length,
              (_) => variablesMap[key]!.$1,
            );
    ...
    entities.removeWhere((e) => e.path.contains(".DS_Store"));
    //edit file content
    for (var entity in entities) {
      if (entity is File) {
        var content = entity.readAsStringSync();
        var filePath = entity.path;
        for (var key in variablesMapExtend.keys) {
        ...
        if (filePath != entity.path) {
          ConsoleUtils.echoTextLine(messageLine: "write new file: $filePath");
          FileUtils.writeTextFile(path: filePath, content: content);
          ConsoleUtils.echoTextLine(messageLine: "delete: ${entity.path}");
          FileUtils.deleteFile(entity.path);
        }
      }
    }
    for (var entity in entities) {
      if (entity is Directory) {
        ...
        if (dirPath != entity.path) {
          ConsoleUtils.echoTextLine(messageLine: "write new dir: $dirPath");
