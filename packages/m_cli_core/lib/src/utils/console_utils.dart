...
class ConsoleUtils {
  ...
  static void echoSucceed(String message) {
    return echoText(message: message, type: TextType.success);
  }

  static void echoError(String message) {
    return echoText(message: message, type: TextType.error);
  }

  static void echoWarn(String message) {
    return echoText(message: message, type: TextType.warn);
  }

  static void echo(String message) {
    return echoText(message: message);
  }

  static void echoLineSucceed(String message) {
    return echoTextLine(messageLine: message, type: TextType.success);
  }

  static void echoLineError(String message) {
    return echoTextLine(messageLine: message, type: TextType.error);
  }

  static void echoLineWarn(String message) {
    return echoTextLine(messageLine: message, type: TextType.warn);
  }

  static void echoLine(String message) {
    return echoTextLine(messageLine: message);
  }
  ...
  static String? echoAndRead({
    ...
    if (allowNullOrEmptyInput == false &&
      ...
      echoTextLine(type: TextType.warn, messageLine: "Please input a value!");
  ...
  static (bool, bool) echoConfirmYnNnA({
    required String message,
    String confirmHint = "(y/n) or (y all/ n all)",
    List<String>? keysConfirm,
    List<String>? keysConfirmAll,
  }) {
    final userKeysConfirm = keysConfirm ?? ["y", "yes"];
    final userKeysConfirmAll = keysConfirm ?? ["y all", "yes all"];
    final userInput = echoAndRead(messageOut: message);
    if (userKeysConfirmAll.contains(userInput?.toLowerCase())) {
      return (true, false);
    }
    if (userKeysConfirm.contains(userInput?.toLowerCase())) {
      return (false, true);
    }
    return (false, false);
  }
